    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            if (isset($tableSchema->columns['status'])) {
                $files[] = new CodeFile(
                    Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/query/' . $modelClassName . 'Query.php',
                    $this->render('query/query.php', $params)
                );
            }
            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/search/index.html',
                $this->render('search/search.php', [])
            );
