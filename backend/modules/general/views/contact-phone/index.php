...
/* @var $searchModel backend\modules\general\models\search\ContactPhoneSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Yii::t('backend', 'Contact Phone');
$this->params['breadcrumbs'][] = ['label' => 'General', 'url' => ['']];
...
<section id="dom">
    ...
    [
                        ...
                        <div style="margin-top:5px;border:1px solid #ccc;border-radius:3px">
                            <?php Pjax::begin(['id' => 'GeneralContact', 'timeout' => false, 'enablePushState' => false, 'clientOptions' => ['method' => 'GET']]); ?>
                            <?= MyGridView::widget([
                                ...
                                'columns' => [
                                    ...
                                    [
                                        ...
                                        'format' => 'raw',
                                        'value' => function ($model) {
                                            return Html::a($model->phone, 'javascript:void(0)', ['onclick' => 'mycall.makeCall(\'' . $model->phone . '\')']);
                                        }
                                    ],
                                    'description:html',
                                    [
...
$script = <<< JS
var general = new myGridView({
    pjaxId: '#GeneralContact',
    urlChangePageSize: '$urlChangePageSize',
});
$('body').on('beforeSubmit', 'form#form-contact', function(e){
    e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-contact').serialize();
    
    $('#form-contact').myLoading({opacity: true});
    ...
    .done(function(res) {
        $('#form-contact').myUnloading();
        if (res.status == 200) {
            $.when($.pjax.reload({url: currentUrl, method: 'POST', container: general.options.pjaxId})).done(function(){
              ...
              $.ajax({
                ...
                success: function(data){
                    if(data.status == 'success') {
                        ...
                        $.pjax.reload({url: currentUrl, method: 'POST', container: general.options.pjaxId});
                    }
