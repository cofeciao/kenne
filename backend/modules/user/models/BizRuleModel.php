...
class BizRuleModel extends Model
{
    ...
    public function rules(): array
    {
        return [
            [['name', 'className'], 'trim'],
            [['name', 'className', 'actionUpdate', 'rolesParent'], 'required'],
            ['className', 'string'],
            [['name', 'actionUpdate', 'rolesParent'], 'string', 'max' => 64],
    ...
    public function classExists()
    {
        if (!class_exists($this->className)) {
            $message = Yii::t('rbac', "Unknown class '{class}'", ['class' => $this->className]);
        ...
        if (!is_subclass_of($this->className, Rule::class)) {
            $message = Yii::t('rbac', "'{class}' must extend from 'yii\\rbac\\Rule' or its child class", [
