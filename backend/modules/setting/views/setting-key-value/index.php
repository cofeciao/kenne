...
/* @var $searchModel backend\modules\setting\models\search\SettingSearch */
...
<section id="dom">
    ...
    [
                        ...
                        <div style="margin-top:5px;border:1px solid #ccc;border-radius:3px">
                            <?php Pjax::begin(['id' => 'SetSetting', 'timeout' => false, 'enablePushState' => true, 'clientOptions' => ['method' => 'GET']]); ?>
                            <?= MyGridView::widget([
...
$script = <<< JS
var setSetting = new myGridView({
    pjaxId: '#SetSetting',
    urlChangePageSize: '$urlChangePageSize',
});
$('body').on('beforeSubmit', 'form#form-setting-kv', function(e) {
   e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-setting-kv').serialize();

    $('#form-setting-kv').myLoading({opacity: true});
   
    $.ajax({
        url: $('#form-setting-kv').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        if (res.status == 200) {
            $.when($.pjax.reload({url: currentUrl, method: 'POST', container: setSetting.options.pjaxId})).done(function(){
              ...
              $.ajax({
                ...
                success: function(data){
                    if(data.status == 'success') {
                        ...
                        $.pjax.reload({url: currentUrl, method: 'POST', container: setSetting.options.pjaxId});
                    }
