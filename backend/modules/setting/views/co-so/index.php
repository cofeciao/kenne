...
/* @var $searchModel backend\modules\setting\models\search\Dep365CoSoSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Yii::t('backend', 'Cơ sở');
...
<section id="dom">
    ...
    [
                        ...
                        <div style="margin-top:5px;border:1px solid #ccc;border-radius:3px">
                            <?php Pjax::begin(['id' => 'SetCoso', 'timeout' => false, 'enablePushState' => true, 'clientOptions' => ['method' => 'GET']]); ?>
                            <?= MyGridView::widget([
...
//                                'filterModel' => $searchModel,
                                ...
                                'columns' => [
                                    ...
                                    [
                                        'attribute' => 'render_virtual_booking',
                                        'label' => 'Lịch ảo',
                                        'format' => 'raw',
                                        'value' => function ($model) {
                                            return \common\widgets\ModavaCheckbox::widget([
                                                'id' => $model->id,
                                                'value' => $model->render_virtual_booking,
                                                'className' => 'toggle-render-booking'
                                            ]);
                                        },
                                        'headerOptions' => [
                                            'width' => 70,
...
$urlRenderVirtual = \yii\helpers\Url::toRoute(['update-render-virtual']);
...
$script = <<< JS
var setCoso = new myGridView({
    pjaxId: '#SetCoso',
    urlChangePageSize: '$urlChangePageSize',
});
$('body').on('beforeSubmit', 'form#form-setting-coso', function(e) {
   e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-setting-coso').serialize();

    $('#form-setting-coso').myLoading({opacity: true});
   
    $.ajax({
        url: $('#form-setting-coso').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        $('#form-setting-coso').myUnloading();
        if (res.status == 200) {
            $.when($.pjax.reload({url: currentUrl, method: 'POST', container: setCoso.options.pjaxId})).done(function(){
...
$(document).ready(function () {
    $('.check-toggle:not(.toggle-render-booking)').change(function () {
        var id = $(this).val();
    ...
    $('.toggle-render-booking').change(function () {
        var id = $(this).val();
        $.post('$urlRenderVirtual', {id: id}, function (result) {
            if(result == 1) {
                toastr.success('$resultSuccess', '$tit');
            }
            if(result == 0) {
              ...
              $.ajax({
                ...
                success: function(data){
                    if(data.status == 'success') {
                        ...
                        $.pjax.reload({url: currentUrl, method: 'POST', container: setCoso.options.pjaxId});
                    }
