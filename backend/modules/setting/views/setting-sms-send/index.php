...
/* @var $searchModel backend\modules\setting\models\search\Dep365SettingSmsSendSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Yii::t('backend', 'Cài đặt tin nhắn gửi');
...
<section id="dom">
    ...
    [
                        ...
                        <div style="margin-top:5px;border:1px solid #ccc;border-radius:3px">
                            <?php Pjax::begin(['id' => 'SetSms', 'timeout' => false, 'enablePushState' => true, 'clientOptions' => ['method' => 'GET']]); ?>
                            <?= MyGridView::widget([
                                ...
                                'columns' => [
                                    ...
                                    [
                                        ...
                                        'value' => function ($model) {
                                            $user = new backend\modules\setting\models\Dep365SettingSmsSend();
...
$script = <<< JS
var setSms = new myGridView({
    pjaxId: '#SetSms',
    urlChangePageSize: '$urlChangePageSize',
});
$('body').on('beforeSubmit', 'form#form-setting-sms', function(e) {
   e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-setting-sms').serialize();

    $('#form-setting-sms').myLoading({opacity: true});
   
    $.ajax({
        url: $('#form-setting-sms').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        $('#form-setting-sms').myUnloading();
        if (res.status == 200) {
            $.when($.pjax.reload({url: currentUrl, method: 'POST', container: setSms.options.pjaxId})).done(function(){
              ...
              $.ajax({
                ...
                success: function(data){
                    if(data.status == 'success') {
                        ...
                        $.pjax.reload({url: currentUrl, method: 'POST', container: setSms.options.pjaxId});
                    }
