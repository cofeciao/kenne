...
/* @var $searchModel backend\modules\location\models\search\WardSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Yii::t('backend', 'Ward');
...
<section id="dom">
    ...
    [
                        ...
                        <div style="margin-top:5px;border:1px solid #ccc;border-radius:3px">
                            <?php Pjax::begin(['id' => 'locationWard', 'timeout' => false, 'enablePushState' => true, 'clientOptions' => ['method' => 'GET']]); ?>
                            <?= MyGridView::widget([
                            ...
                            'columns' => [
                                ...
                                [
                                    ...
                                    'attribute' => 'Type',
                                    'enum' => Ward::getWardType(),
                                    'filter' => Ward::getWardType(),
                                ],
                                'LatiLongTude',
                                'quanHuyen.name',
                                'SortOrder',
...
<?php
$url = \yii\helpers\Url::toRoute(['/location/ward/show-hide']);
$urlDelete = \yii\helpers\Url::toRoute(['/location/ward/delete']);
$urlChangePageSize = \yii\helpers\Url::toRoute(['/location/ward/perpage']);
...
$script = <<< JS
var locationWard = new myGridView({
    pjaxId: '#locationWard',
    urlChangePageSize: '$urlChangePageSize' 
});
$('body').on('beforeSubmit', 'form#form-location-ward', function(e) {
   e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-location-ward').serialize();

    $('#form-location-ward').myLoading({opacity: true});
   
    $.ajax({
        url: $('#form-location-ward').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        if (res.status == 200) {
            $.when($.pjax.reload({url: currentUrl, method: 'POST', container: locationWard.options.pjaxId})).done(function(){
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                           if(data.status == 'success') {
                               ...
                               table.slideUp("slow");
                               $.pjax.reload({url: currentUrl, method: 'POST', container: locationWard.options.pjaxId});
                           }
