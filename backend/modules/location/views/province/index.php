...
/* @var $searchModel backend\modules\location\models\search\ProvinceSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Yii::t('backend', 'Province');
...
<section id="dom">
    ...
    [
                        ...
                        <div style="margin-top:5px;border:1px solid #ccc;border-radius:3px">
                            <?php Pjax::begin(['id' => 'locationProvince', 'timeout' => false, 'enablePushState' => true, 'clientOptions' => ['method' => 'GET']]); ?>
                            <?= MyGridView::widget([
                                ...
                                'columns' => [
                                    ...
                                    [
                                        ...
                                        'attribute' => 'Type',
                                        'enum' => Province::provinceType(),
                                        'filter' => Province::provinceType(),
                                    ],
                                    'TelephoneCode',
                                    'ZipCode',
                                    'country.CommonName',
                                    ...
                                    [
                                        ...
                                        'value' => function ($model) {
                                            $user = new backend\modules\location\models\Province();
...
<?php
$url = \yii\helpers\Url::toRoute(['/location/province/show-hide']);
$urlDelete = \yii\helpers\Url::toRoute(['/location/province/delete']);
$urlChangePageSize = \yii\helpers\Url::toRoute(['/location/province/perpage']);
...
$script = <<< JS
var locationProvince = new myGridView({
    pjaxId: '#locationProvince',
    urlChangePageSize: '$urlChangePageSize',
});
$('body').on('beforeSubmit', 'form#form-location-province', function(e) {
   e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-location-province').serialize();

    $('#form-location-province').myLoading({opacity: true});
   
    $.ajax({
        url: $('#form-location-province').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        $('#form-location-province').myUnloading();
        if (res.status == 200) {
            $.when($.pjax.reload({url: currentUrl, method: 'POST', container: locationProvince.options.pjaxId})).done(function(){
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                           if(data.status == 'success') {
                               ...
                               table.slideUp("slow");
                               $.pjax.reload({url: currentUrl, method: 'POST', container: locationProvince.options.pjaxId});
                           }
