                ...
                <div class="card-content collapse show">
                    <div class="card-body card-dashboard">
                        <div style="margin-top:5px;border:1px solid #ccc;border-radius:3px">
                            <?php Pjax::begin(['id' => 'locationCountry', 'timeout' => false, 'enablePushState' => true, 'clientOptions' => ['method' => 'GET']]); ?>
                            <?= MyGridView::widget([
...
//                                'CountryNumber',
                                    ...
                                    [
                                        ...
                                        'value' => function ($model) {
                                            $user = new backend\modules\location\models\Country();
...
<?php
$url = \yii\helpers\Url::toRoute(['/location/country/show-hide']);
$urlDelete = \yii\helpers\Url::toRoute(['/location/country/delete']);
$urlChangePageSize = \yii\helpers\Url::toRoute(['/location/country/perpage']);
...
$script = <<< JS
var locationCountry = new myGridView({
    pjaxId: '#locationCountry',
    urlChangePageSize: '$urlChangePageSize',
});
$('body').on('beforeSubmit', 'form#form-location-country', function(e) {
   e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-location-country').serialize();

    $('#form-location-country').myLoading({opacity: true});
   
    $.ajax({
        url: $('#form-location-country').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        $('#form-location-country').myUnloading();
        if (res.status == 200) {
            $.when($.pjax.reload({url: currentUrl, method: 'POST', container: locationCountry.options.pjaxId})).done(function(){
              ...
              $.ajax({
                ...
                success: function(data){
                    if(data.status == 'success') {
                        ...
                        $.pjax.reload({url: currentUrl, method: 'POST', container: locationCountry.options.pjaxId});
                    }
