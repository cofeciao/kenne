 ...
 */
class MyAccessControl extends \yii\filters\AccessControl
{
    /**
     * @var array
     */
    ...
    public function beforeAction($action): bool
    {
        ...
        if (MyHelper::containsWord($controller->getUniqueId(), '/')) {
            ...
            $canModules = $modules[0] . ucfirst($modules[1]);
            if (Yii::$app->user->can($canModules, $params)) {
                return true;
            }
            // Kiểu modules
            $canAll = $modules[0];
        } else {
            $can = $controller->getUniqueId() . ucfirst($action->id);
            $canAll = $controller->getUniqueId();
        }
        if (Yii::$app->user->can($can, $params) || Yii::$app->user->can($canAll, $params)) {
            return true;
        }
